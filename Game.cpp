...
void Game::setup() 
{
	...
	game_state_manager = new cotw::GameStateManager;
}

void Game::handle_events()
{
	game_state_manager->state->handle_events();
}

void Game::update() 
{
	game_state_manager->state->update(window);
}
...
void Game::draw(sf::RenderTarget& render_target, sf::RenderStates render_states) //const
{
	...
	game_state_manager->state->draw(render_target, render_states);
	bitmap.display();
...
int Game::game_loop()
{
    ...
    while (window.isOpen())
    {
		...
		handle_events();
		update();
		sf::RenderStates render_states1;
		draw(bitmap, render_states1);

		//cout << static_cast<cotw::MainMenuState*>(gstate)->foo << endl;
		//cout << gstate->foo << endl;
